...
def random_agent_ob_mean_std(env, env_name, nsteps=10000, force_reset=None, load=False):
    # ob = env.reset()
    # ob_shape=ob.shape
    # ob_space, ac_space = env.observation_space, env.action_space
    # np.savetxt(env_name+'_ac_space.txt', np.array([ac_space.n]))
    if load:
        ob_shape = np.loadtxt(env_name+'_ob_shape.txt')
        ob_shape = [int(i) for i in ob_shape]
        ob_space = gym.spaces.Box(0, 255, ob_shape, dtype=np.uint8)
        ac_space = np.loadtxt(env_name+'_ac_space.txt')
        ac_space = gym.spaces.Discrete(int(ac_space))
        mean = np.loadtxt(env_name+'_mean.txt')
        mean = mean.reshape(ob_shape)
        std = np.loadtxt(env_name+'_std.txt')
    else:
        ob_space, ac_space = env.observation_space, env.action_space
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:
            ...
            for _ in range(nsteps):
                ...
                ep_step += 1
                if force_reset is not None and ep_step % force_reset == 0:
                    done = True
                if done:
                    ob = env.reset()
                    ep_step = 0
            ...
            with open(env_name+'_mean.txt', 'w') as outfile:
                outfile.write('# Array shape: {0}\n'.format(mean.shape))
                for slice_2d in mean:
                    np.savetxt(outfile, slice_2d)
                    outfile.write('# New slice\n')
            np.savetxt(env_name+'_std.txt', np.array([std]))
            np.savetxt(env_name+'_ob_shape.txt', np.array([ob.shape]))
            np.savetxt(env_name+'_ac_space.txt', np.array([ac_space.n]))
        else:
    ...
    return mean, std, ob_space, ac_space
