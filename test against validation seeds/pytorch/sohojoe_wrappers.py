...
class TrainLevelWrapper(gym.Wrapper):
    ...
    Arguments:
      ...
      reset_mode: 
            ...
            'seed_from_10'=start level 10 with one seed then do moe
    """
    def __init__(self, env, reset_mode='0', show_debug=False):
        gym.Wrapper.__init__(self, env)
    ...
    def step(self, action):
        ...
        if show_debug and len(info_str) > 0:
            if done:
                ...
                print (info_str + 'reward: ' + str(np.around(reward,1)))
        return ob, reward, done, info   
...
class RenderObservations(gym.Wrapper):
    ...
    def _renderObs(self, obs, should_render):
        ...
        _8bit = ( visual_obs).astype(np.uint8)
        self.viewer.imshow(_8bit)
        return self.viewer.isopen
...
class KeyboardControlWrapper(gym.ActionWrapper):
    def __init__(self, env, set_human_control=False):
        ...
        global human_agent_action, human_wants_restart, human_sets_pause, human_has_control, num_actions, human_agent_display, up_key
    ...
    def action(self, action):
        ...
        if human_has_control:
            while time.time()-self._last_time < 1/15.:
                time.sleep(0.01)
            self._last_time = time.time()
...
class NormalizeWrapper(gym.ObservationWrapper):
    def __init__(self, env, normal_filename):
        ...
        is_8bit = True # HACK
        # ob_shape = np.loadtxt(normal_filename+'_ob_shape.txt')
        # ob_shape = [int(i) for i in ob_shape]
        # ob_space = gym.spaces.Box(0, 255, ob_shape, dtype=np.uint8)
        # ac_space = np.loadtxt(normal_filename+'_ac_space.txt')
        # ac_space = gym.spaces.Discrete(int(ac_space))
