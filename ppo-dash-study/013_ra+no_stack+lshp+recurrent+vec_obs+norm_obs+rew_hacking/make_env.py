...
def make_env_all_params(rank, env_count, total_env, add_monitor, args, sleep_multiple=3):
    ...
    if args.env == 'ObtRetro-baseline':
    ...
    elif args.env == 'ObtRetro-ra+no_stack+lshp+recurrent+vec_obs+norm_obs+rew_hacking':
        env = ObstacleTowerEnv(
            environment_path,
            worker_id=rank,
            timeout_wait=6000,
        ...
        if not is_grading and not args.sample_normal:
            if args.score:
                env = ScoreLevelWrapper(env)
            env = TrainLevelWrapper(env, env_count, reward_hacking=args.reward_hacking)
...
def otc_arg_parser():
    ...
    parser.add_argument(
        '--env',
        default='ObtRetro-ra+no_stack+lshp+recurrent+vec_obs+norm_obs+rew_hacking', #'PongNoFrameskip-v4',
    ...
    parser.add_argument('--reward_hacking', action='store_true')
    parser.set_defaults(reward_hacking=True)
    return parser
