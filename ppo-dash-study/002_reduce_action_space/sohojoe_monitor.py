...
class Monitor(Wrapper):
    ...
    def step(self, action):
        ...
        action_idx = len(self.actions)-1
        if action_idx < 0:
            action_idx=0
            action_tuple=(action,0)
            self.actions = [(action_tuple)]
        else:
            action_tuple=self.actions[action_idx]
        if action_tuple[0] is not action:
            action_tuple=(action,0)
            action_idx += 1
            self.actions.append(action_tuple)
        action_tuple = (action_tuple[0], action_tuple[1]+1)
        self.actions[action_idx] = action_tuple
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if done and not self.env_semantics_autoreset and self.video_recorder:
            self._close_video_recorder()
            self.video_recorder = None
    ...
    def reset_video_recorder(self):
        ...
        self.video_recorder = video_recorder.VideoRecorder(
            ...
            base_path=os.path.join(self.directory, '{}.video.{}.floor{:02}.seed{:02}'.format(self.file_prefix, self.file_infix, self.get_floor, self.get_seed)),
    ...
    def _close_video_recorder(self):
        self.video_recorder.metadata['actions'] = self.actions
