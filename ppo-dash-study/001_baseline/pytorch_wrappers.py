...
# # does not handle vector arguments
...
class VecPyTorchFrameStack(VecEnvWrapper):
    ...
    def step_wait(self):
        obs, vector_obs, reward, done, info = self.venv.step_wait()
        self.stacked_obs[:, :-self.shape_dim0] = \
            self.stacked_obs[:, self.shape_dim0:]
        for (i, new) in enumerate(done):
            if new:
                self.stacked_obs[i] = 0
        self.stacked_obs[:, -self.shape_dim0:] = obs
        return self.stacked_obs, vector_obs, reward, done, info

    def reset(self):
        obs, vector_obs = self.venv.reset()
        if torch.backends.cudnn.deterministic:
            self.stacked_obs = torch.zeros(self.stacked_obs.shape)
        else:
            self.stacked_obs.zero_()
        self.stacked_obs[:, -self.shape_dim0:] = obs
        return self.stacked_obs, vector_obs
    ...
    def vector_obs_len(self):
        return self.venv._vector_obs_len
